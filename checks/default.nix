    ...
    in {
      ...
      neovim-check-config =
        pkgs.runCommand "neovim-check-config"
        {
          ...
          # need git in path
          export HOME=$TMPDIR
          ${self'.packages.neovim-polar}/bin/nvim --headless -c "q" 2> "$out/nvim-config.log"
      ...
      neovim-check-health =
        pkgs.runCommand "neovim-check-health"
        {
          ...
          # need git in path
          export HOME=$TMPDIR
          ${self'.packages.neovim-polar}/bin/nvim --headless -c "lua require('polarmutex.health').nix_check()" -c "q" 2> "$out/nvim-health.log"
